...
static int i2s_esp32_restart_dma(const struct device *dev, enum i2s_dir dir);
	...
	i2s_hal_get_out_eof_des_addr(hal, (uint32_t *)&finish_desc);
...
#if SOC_GDMA_SUPPORTED /*TODO:*/
	err = dma_start(dev, stream->conf->dma_channel);
	if (err < 0) {
		LOG_ERR("Error starting DMA channel[%"PRIu32"]: %d", stream->conf->dma_channel,
			err);
	}
#else
	i2s_ll_enable_dma(hal->dev, true);
#endif /* SOC_GDMA_SUPPORTED */
	if (err < 0) {
		ret = -EIO;
		goto unlock;
...
LOG_INF("i2s_esp32_rx_stop_transfer()");
	...
	/* esp_intr_disable(stream->conf->dma_channel); */
#endif /* SOC_GDMA_SUPPORTED */
...
static int i2s_esp32_initialize(const struct device *dev)
{
	...
	if (dev_cfg->rx.conf->dma_dev || dev_cfg->rx.conf->irq_source) {
	...
	if (dev_cfg->tx.conf->dma_dev || dev_cfg->tx.conf->irq_source) {
	...
	/* i2s_ll_clear_intr_status(); <-- */

	if (dev_cfg->rx.conf->dma_dev || dev_cfg->rx.conf->irq_source) {
		int irq_flags = ESP_PRIO_TO_FLAGS(dev_cfg->rx.conf->irq_priority) |
				ESP_INT_FLAGS_CHECK(dev_cfg->rx.conf->irq_flags) |
				ESP_INTR_FLAG_IRAM | ESP_INTR_FLAG_SHARED;

		err =  esp_intr_alloc_intrstatus(dev_cfg->rx.conf->irq_source, irq_flags,
	...
	if (dev_cfg->tx.conf->dma_dev || dev_cfg->tx.conf->irq_source) {
		int irq_flags = ESP_PRIO_TO_FLAGS(dev_cfg->tx.conf->irq_priority) |
				ESP_INT_FLAGS_CHECK(dev_cfg->tx.conf->irq_flags) |
				ESP_INTR_FLAG_IRAM | ESP_INTR_FLAG_SHARED;

		err =  esp_intr_alloc_intrstatus(dev_cfg->tx.conf->irq_source, irq_flags,
						 (uint32_t)i2s_ll_get_intr_status_reg(hal->dev),
...
#if SOC_I2S_HW_VERSION_2
		if (dev_cfg->tx.data->state != I2S_STATE_NOT_READY) {
			if (stream->data->is_slave && dev_cfg->rx.data->is_slave) {
...
#if SOC_I2S_HW_VERSION_2
		if (dev_cfg->tx.data->state != I2S_STATE_NOT_READY) {
			if (!stream->data->is_slave && !dev_cfg->tx.data->is_slave) {
...
#define I2S_ESP32_STREAM_DECL(index, dir)                                                          \
	struct i2s_esp32_stream_data stream_##dir##_data = {                                       \
		...
		.queue = {}                                                                        \
                                                                                                   ...
                                                                                                   \
	const struct i2s_esp32_stream_conf stream_##dir##_conf = {                                 \
		...
		.irq_source = UTIL_AND(DT_INST_IRQ_HAS_NAME(index, dir),                           \
				       DT_INST_IRQ_BY_NAME(index, dir, irq)),                      \
		.irq_priority = UTIL_AND(DT_INST_IRQ_HAS_NAME(index, dir),                         \
					 DT_INST_IRQ_BY_NAME(index, dir, priority)),               \
		.irq_flags = UTIL_AND(DT_INST_IRQ_HAS_NAME(index, dir),                            \
				      DT_INST_IRQ_BY_NAME(index, dir, flags))                      \
	}

#define I2S_ESP32_STREAM_INIT(index, dir)                                                          \
	.dir = {.conf = &stream_##dir##_conf,                                                      \
		.data = &stream_##dir##_data}

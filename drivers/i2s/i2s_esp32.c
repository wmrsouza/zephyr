...
LOG_INF("i2s_esp32_config_dma():%"PRIu32, ++i2s_esp32_config_dma_counter);
	...
	int err = dma_config(stream->conf->dma_dev, stream->conf->dma_channel, &dma_cfg);
	if (err < 0) {
	...
	for (int i = 0; i < CONFIG_I2S_ESP32_DMA_DESC_NUM_MAX; ++i) {
		...
		if (mem_block_size > 0) {
			...
			stream->data->dma_pending_descs = i + 1;
			desc_iter->empty = 0;
			if (dir == I2S_DIR_TX) {
				desc_iter->eof = 1;
			}
	...
	if (desc_iter->empty)  {
		stream->data->dma_pending_descs = 0;
...
LOG_INF("i2s_esp32_initialize():%"PRIu32, ++i2s_esp32_initialize_counter);
	...
	if (dev_cfg->rx.data && dev_cfg->rx.conf) {
		...
		err = esp_intr_alloc_intrstatus(dev_cfg->rx.conf->irq_source, irq_flags,
						...
						(void *)dev, &(dev_cfg->rx.data->irq_handle));
		if (err != 0) {
	...
	if (dev_cfg->tx.data && dev_cfg->tx.conf) {
		...
		err = esp_intr_alloc_intrstatus(dev_cfg->tx.conf->irq_source, irq_flags,
						...
						(void *)dev, &(dev_cfg->tx.data->irq_handle));
		if (err != 0) {
...
LOG_INF("i2s_esp32_trigger():%"PRIu32, ++i2s_esp32_trigger_counter);
	...
	case I2S_TRIGGER_STOP:
		...
		dma_busy = (bool)dma_channel_status.busy;
#else
		dma_busy = (bool)stream->data->dma_pending_descs;
#endif /* SOC_GDMA_SUPPORTED */
	...
	case I2S_TRIGGER_DRAIN:
		...
		if (dir == I2S_DIR_TX) {
			if (k_msgq_num_used_get(&stream->data->queue) > 0 || dma_busy) {

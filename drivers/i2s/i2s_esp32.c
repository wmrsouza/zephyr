...
#endif /* SOC_GDMA_SUPPORTED */
{
	...
	if (dev_data->state == I2S_STATE_STOPPING) {
		if (!dev_cfg->tx.data->transferring) {
...
void i2s_esp32_tx_compl_transfer(struct k_timer *timer)
{
	...
	if (dev_data->state == I2S_STATE_STOPPING) {
		if (k_msgq_num_used_get(&stream->data->queue) == 0 ||
			...
			if (!dev_cfg->rx.data->transferring) {
				dev_data->state = I2S_STATE_READY;
			}
			goto tx_disable;
		}
...
static bool i2s_esp32_try_stop_transfer(const struct device *dev, enum i2s_dir dir, enum i2s_trigger_cmd cmd)
{
	const struct i2s_esp32_cfg *dev_cfg = dev->config;
	const struct i2s_esp32_stream *stream;
			       ...
			       const struct i2s_config *i2s_cfg)
{
	...
	if (dir == I2S_DIR_BOTH) {
		if (!dev_cfg->rx.conf || !dev_cfg->rx.data ||
		    !dev_cfg->tx.conf || !dev_cfg->tx.data) {
	...
	if (i2s_cfg->options & I2S_OPT_LOOPBACK) {
		...
		LOG_DBG("To enable loopback, use the same pin SD pinctrl for TX and RX pinctrl");
		return -EINVAL;
	}
	...
	int err = i2s_esp32_calculate_clock(i2s_cfg, slot_cfg.slot_bit_width, &i2s_hal_clock_info);

	if (err != ESP_OK) {
		return -EINVAL;
	}

	if (dev_cfg->full_duplex) { //<-- considerar I2S_DIR_BOTH
	...
	if (dir == I2S_DIR_RX || dir == I2S_DIR_BOTH) {
		...
		i2s_hal_std_set_rx_slot(hal, is_slave, &slot_cfg);
		i2s_hal_set_rx_clock(hal, &i2s_hal_clock_info, I2S_ESP32_CLK_SRC);
		i2s_ll_rx_enable_std(hal->dev);

#if SOC_I2S_HW_VERSION_2
		if (!is_slave && !dev_cfg->full_duplex) { //<-- (?) considerar I2S_DIR_BOTH
		...
		if (!dev_cfg->full_duplex) { // <-- investigar consequencias
			dev_cfg->tx.data->configured = false;
			// (?) necessario desabilitar alguma coisa
		}
	}
#endif /* I2S_ESP32_IS_DIR_EN(rx) */
	...
	if (dir == I2S_DIR_TX || dir == I2S_DIR_BOTH) {
		...
		stream->data->configured = true;
		if (!dev_cfg->full_duplex) { // <-- investigar consequencias
			dev_cfg->rx.data->configured = false;
			// (?) necessario desabilitar alguma coisa
		}
	}
#endif /* I2S_ESP32_IS_DIR_EN(tx) */
...
static int i2s_esp32_trigger(const struct device *dev, enum i2s_dir dir, enum i2s_trigger_cmd cmd)
{
	struct i2s_esp32_data *dev_data = dev->data;
	const struct i2s_esp32_cfg *dev_cfg = dev->config;
	bool configured, cmd_allowed, at_least_one_dir_with_pending_transfer;
	unsigned int key;
	int err;
	...
	case I2S_TRIGGER_START:
		cmd_allowed = (dev_data->state == I2S_STATE_READY);
		break;
	case I2S_TRIGGER_STOP:
		__fallthrough;
	case I2S_TRIGGER_DRAIN:
		cmd_allowed = (dev_data->state == I2S_STATE_RUNNING);
		break;
	case I2S_TRIGGER_DROP:
		cmd_allowed = configured;
		break;
	case I2S_TRIGGER_PREPARE:
		cmd_allowed = (dev_data->state == I2S_STATE_ERROR);
		break;
	default:
	...
	if (!cmd_allowed) {
		switch (cmd) {
		case I2S_TRIGGER_START:
			LOG_DBG("START - Invalid state: %d", (int)dev_data->state);
			break;
		case I2S_TRIGGER_STOP:
			LOG_DBG("STOP - Invalid state: %d", (int)dev_data->state);
			break;
		case I2S_TRIGGER_DRAIN:
			LOG_DBG("DRAIN - Invalid state: %d", (int)dev_data->state);
			break;
		case I2S_TRIGGER_DROP:
			LOG_DBG("DROP - invalid state: %d", (int)dev_data->state);
			break;
		case I2S_TRIGGER_PREPARE:
			LOG_DBG("PREPARE - invalid state: %d", (int)dev_data->state);
		}
